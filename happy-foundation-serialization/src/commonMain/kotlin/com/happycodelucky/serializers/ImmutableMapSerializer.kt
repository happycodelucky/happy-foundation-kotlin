...
class ImmutableMapSerializer<K, V>(
    ...
    override fun serialize(encoder: Encoder, value: ImmutableMap<K, V>) {
        return MapSerializer(kSerializer, vSerializer).serialize(encoder, value)
    }
    override fun deserialize(decoder: Decoder): ImmutableMap<K, V> {
        return MapSerializer(kSerializer, vSerializer).deserialize(decoder).toPersistentHashMap()
    }
}

...
class FlowCoalesceTests {
    ...
    @Test
    fun `single item emits as single-item list on completion`() =
        runTest {
            val timeSource = TestTimeSource()
    ...
    @Test
    fun `items within window are batched together on completion`() =
        runTest {
            val timeSource = TestTimeSource()
    ...
    @Test
    fun `items emitted after window expires are in separate batches`() =
        runTest {
            val timeSource = TestTimeSource()
    ...
    @Test
    fun `window resets after each emission allowing multiple batches`() =
        runTest {
            val timeSource = TestTimeSource()
            val window = 50.milliseconds
    ...
    @Test
    fun `remaining items emitted when upstream completes`() =
        runTest {
            val timeSource = TestTimeSource()
    ...
    @Test
    fun `rapid emissions within window are all batched`() =
        runTest {
            val timeSource = TestTimeSource()
